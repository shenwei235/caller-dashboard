...
function qiniuUploader(options) {
	...
	this.setPath = function(publicPath, filePath) {
	...
	this.buildPathRegex = function(name) {
		return (this.publicPath + name).split('\/').reduce(function(arr, item) {
			if(item.trim()) {
				arr.push(item);
			}
			return arr;
	...
	this.replaceContentsFromFile = function(path, names, url_prefix, cb) {
		...
		fs.readFile(path, 'utf8', function(err, data) {
		  	...
		  	names.map(function(name, index) {
		  		// Since Webpack 2+ generate file with assets path, 
		  		// Build the regex with publicPath + name...
